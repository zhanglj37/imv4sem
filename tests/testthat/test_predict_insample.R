...
# Test the predict_insample function
test_that("predict_insample correctly predicts probabilities", {
  ...
  # Call predict_insample
  yprob <- predict_insample(data = data, model = model, vary = vary)
  ...
  expect_equal(nrow(yprob), nrow(data), info = "Number of rows in the output should match the number of rows in 'data'.")
